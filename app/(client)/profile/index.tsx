...
export default function ClientProfileScreen() {
  ...
  const coachCount = coachesQuery.data?.length ?? 0;

  const saveProfileMutation = trpc.users.saveProfile.useMutation({
    onError: (error) => console.error('[ClientProfile] saveProfile failed:', error.message),
  });

  const initials = useMemo(() => {
    const name = userName ?? 'C';
        ...
        >
          ...
          {isEditing ? (
            ...
            <Pressable onPress={() => setIsEditing(true)} style={styles.nameRow}>
              <Text style={styles.displayName}>{userName ?? 'Client'}</Text>
        ...
        <View style={styles.content}>
          <View style={styles.statCard}>
            ...
            <View>
              ...
              <Text style={styles.statLabel}>Connected Coaches</Text>
            </View>
          </View>
