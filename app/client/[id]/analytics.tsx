...
function computeAnalytics(history: Workout[]) {
  const totalSessions = history.length;
  const totalVolume = history.reduce((s, w) => s + w.totalVolume, 0);
  const totalDuration = history.reduce((s, w) => s + w.duration, 0);
  ...
  history.forEach((w) => {
    w.exercises.forEach((ex) => {
      ex.sets.forEach((set) => {
        if (set.completed) {
          if (!prMap[ex.exerciseName] || set.weight > prMap[ex.exerciseName].weight) {
            prMap[ex.exerciseName] = { weight: set.weight, reps: set.reps };
          }
        }
      });
    });
  ...
  return { totalSessions, totalVolume, totalDuration, avgVolume, topExercises, weeklyData, personalRecords };
}
...
export default function ClientAnalyticsScreen() {
  ...
  const analyticsData = useMemo(() => computeAnalytics(history), [history]);
        ...
        {analyticsData.totalSessions === 0 && (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySubtitle}>
              Complete workouts with this client to see analytics here
            </Text>
          </View>
        )}
